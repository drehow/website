                  ...
                  , columns = list(
                        ...
                        ,SD = colDef(aggregate = "sum", maxWidth = 50,
                                     cell = function(value, index) {
                                         ...
                                         bar_chart(value, width = width, fill = "#D32E0B", background = "#e1e1e1")
                                     }
                                     )
                        ,D = colDef(aggregate = "sum", maxWidth = 50,
                                    cell = function(value, index) {
                                        width <- paste0(value / sum(unlist(df_opinion[index,3:7])) * 100, "%")
                                        bar_chart(value, width = width, fill = "#E78F26", background = "#e1e1e1")
                                    })
                        ,N = colDef(aggregate = "sum", maxWidth = 50,
                                    cell = function(value, index) {
                                        width <- paste0(value / sum(unlist(df_opinion[index,3:7])) * 100, "%")
                                        bar_chart(value, width = width, fill = "#B3FFA6", background = "#e1e1e1")
                                    })
                        ,A = colDef(aggregate = "sum", maxWidth = 50,
                                    cell = function(value, index) {
                                        width <- paste0(value / sum(unlist(df_opinion[index,3:7])) * 100, "%")
                                        bar_chart(value, width = width, fill = "#B8E24B", background = "#e1e1e1")
                                    })
                        ,SA = colDef(aggregate = "sum", maxWidth = 50,
                                     cell = function(value, index) {
                                         width <- paste0(value / sum(unlist(df_opinion[index,3:7])) * 100, "%")
                                         bar_chart(value, width = width, fill = "#1EC600", background = "#e1e1e1")
                                     })
                    )
                  ,bordered = TRUE
                  ,highlight = TRUE
